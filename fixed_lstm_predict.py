...
def create_model(mode='1', coin='btc', model_type='MLP', in_seq_len='1', out_seq_len='1'):
    global mae
    print(f"Creating a new {model_type} model...")
    
    if mode == '1':
        df = pd.read_csv(f'{coin}_order_book_summary.csv')
        # USE ALL 4 FEATURES to match depth_chart_live output
        X = df[['Mean Bids Price', 'Mean Asks Price', 'Mean Bids Size', 'Mean Asks Size']].values
    ...
    print(f"Total data rows available: {len(X)}")
    ...
    if model_type == 'MLP':
        ...
        min_required = in_seq_len + out_seq_len
        if len(X) < min_required + 10:
            raise ValueError(f"Not enough data! You have {len(X)} rows but need at least {min_required + 10} rows.\n"
                           f"Required: {in_seq_len} (input seq) + {out_seq_len} (output seq) + 10 (minimum for train/test split)\n"
                           f"Please collect more data by running the depth_chart function in coin_data.py")
        ...
        if modelVar == '1':
            ...
            print("Invalid choice, using basic LSTM")
            model = keras.Sequential([
                layers.Input(shape=(None, X_train.shape[2])),
                layers.LSTM(128, activation='relu'),
                layers.Dense(64, activation='relu'),
    ...
    # Train model
    epochs = 300 if model_type == 'LSTM' else 200
    batch_size = 128 if model_type == 'LSTM' else 100
    ...
    # saving model
    model.save(f'{coin}_{model_type}_{mode}_{in_seq_len}_{out_seq_len}.keras')
    return model

def load_model(mode, coin, model_type, in_seq_len, out_seq_len):
    # Load the saved model
    try:
        model = keras.models.load_model(f'{coin}_{model_type}_{mode}_{in_seq_len}_{out_seq_len}.keras')
        print("Model loaded successfully.")
    except Exception as e:
        print(f"Error loading model: {e}")
        print(f"Creating a new {model_type} model...")
        model = create_model(mode, coin, model_type, in_seq_len, out_seq_len)
    return model
...
def driver():
    ...
    if modelName == 'LSTM' and int(in_seq_len) > 1:
        ...
        data_list = []
        for i in range(int(in_seq_len)):
            data = depth_chart_live(coin)
            data_list.append(data)
            if i < int(in_seq_len) - 1:
                time.sleep(60)
