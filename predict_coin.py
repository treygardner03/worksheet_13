...
def create_model(mode='1',coin='btc'):
    ...
    if tf.config.list_physical_devices('GPU'):
        ...
        early_stop = EarlyStopping(monitor='val_loss', # metric to monitor
        patience=10, # epochs with no improvement to wait before stopping
        restore_best_weights=True # restore model weights from best epoch
        )
        # Train model
        history = model.fit(X_train, y_train,epochs=200,batch_size=100,validation_split=0.05,verbose=1,callbacks=[early_stop])
        # Evaluate model
